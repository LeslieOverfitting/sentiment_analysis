...
class Ernie_poollast3_multidp(nn.Module):
    def __init__(self, bert_model_path, config, seq_len):
        super(Ernie_poollast3_multidp, self).__init__()
    ...
    def forward(self, inputs):
        ...
        cat_inputs = torch.cat([pooler_output, hidden_states[-1][:,0], hidden_states[-2][:,0], hidden_states[-3][:,0]], dim = 1)
        for i, dropout in enumerate(self.dropouts):
            if i == 0:
                out = dropout(cat_inputs)
                out = out.view(out.size(0), -1)
                out = self._predict_fc(out)
            else:
                temp_out = dropout(cat_inputs)
